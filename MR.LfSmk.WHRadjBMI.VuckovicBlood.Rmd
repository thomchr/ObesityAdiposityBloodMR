---
title: "MVMR.LfSmk.WHRadjBMI.VuckovicBlood"
author: "Chris Thom"
date: "2/1/21"
...
setwd("/Users/thomc/Desktop/Smk.BMI.Blood.MR/MVMR.LfSmk.WHRadjBMI.HGB/")
...
if( file.exists("LfSmk.filtered.txt") & file.exists("WHRadjBMI.filtered.txt") & file.exists("HGB.filtered.txt")) { 
  ...
  e2_comm <- fread("WHRadjBMI.filtered.txt", header=T)
  ...
  e1$Phenotype <- "LfSmk"
  e1$ChrPosRefAlt <- paste0(e1$CHR, ":", e1$BP, "_", e1$EFFECT_ALLELE, "_", e1$OTHER_ALLELE)
  e1$ChrPosAltRef <- paste0(e1$CHR, ":", e1$BP, "_", e1$OTHER_ALLELE, "_", e1$EFFECT_ALLELE)
  
  e2 <- fread("/Volumes/4TB.RNAseq/GWASdata/whradjbmi.giant-ukbb.meta-analysis.combined.23May2018.txt", header=T)
  e2$Phenotype <- "WHRadjBMI"
  e2$SNP <- gsub("[A-Z]|:", "", e2$SNP) #bc WHRadjBMI has rsID:Allele:Allele
  ...
  write.table(e2_comm, "WHRadjBMI.filtered.txt", sep="\t", quote= FALSE, row.names = FALSE, col.names = TRUE)
...
if( file.exists("LfSmk.Clumped") & file.exists("WHRadjBMI.Clumped") & file.exists("HGB.Clumped")) { 
  ...
  e2_clump <- fread("WHRadjBMI.Clumped", header=T)
...
write.table(e1_clump, file = "LfSmk.Clumped", sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
...
write.table(e2_clump, file = "WHRadjBMI.Clumped", sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
