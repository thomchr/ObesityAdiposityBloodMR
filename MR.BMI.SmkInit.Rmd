...
setwd("/Users/thomc/Desktop/Smk.BMI.Blood.MR/MR.BMI.SmkInit/")
...
if( file.exists("SmkInit.filtered.txt") & file.exists("BMI.filtered.txt")) { 
  print("Working from pre-tabulated files")
  e1_comm <- fread("BMI.filtered.txt", header=T)
  out_comm <- fread("SmkInit.filtered.txt", header=T)
  ...
  out <- fread("/Volumes/4TB.RNAseq/GWASdata/SmokingInitiation.txt", header=T)
  out$Phenotype <- "SmkInit"
  
  e1_comm <- e1 %>% filter(SNP %in% out$RSID)
  out_comm <- out %>% filter(RSID %in% e1_comm$SNP)
  
  write.table(e1_comm, "BMI.filtered.txt", sep="\t", quote= FALSE, row.names = FALSE, col.names = TRUE)
  write.table(out_comm, "SmkInit.filtered.txt", sep="\t", quote= FALSE, row.names = FALSE, col.names = TRUE)
}
...
if( file.exists("SmkInit.Clumped") & file.exists("BMI.Clumped")) { 
  print("Working from pre-tabulated clump files")
  e1_clump <- fread("BMI.Clumped", header=T)
  out_clump <- fread("SmkInit.Clumped", header=T)
...
e1_data <- format_data(e1_sig, type="exposure", snps=e1_sig$SNP, header=T, phenotype_col = "Phenotype",
                     ...
                     eaf_col="Freq_Tested_Allele_in_HRS", effect_allele_col="Tested_Allele",
                     other_allele_col="Other_Allele", pval_col="P")
e1_clump <- clump_data(e1_data, clump_kb = 500, clump_r2 = 0.01)
write.table(e1_clump, file = "BMI.Clumped", sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)

out_clump <- out_comm %>% filter(RSID %in% e1_clump$SNP)
out_clump <- format_data(out_clump, type="outcome", snps=out_clump$RSID, header=T, phenotype_col = "Phenotype", 
  	      snp_col="RSID", beta_col="BETA", se_col="SE", 
				  eaf_col="AF", effect_allele_col="ALT", other_allele_col="REF", pval_col="PVALUE")
write.table(out_clump, file = "SmkInit.Clumped", sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
...
# sample size for CAD is 547261, SmkInit is 557337 per Dan Hui (paper says up to 1,232,091) --- Using Dan's estimate, BMI and WHRadjBMI are ~700000
dat1$r2 <- 2*(dat1$beta.outcome)^2 * dat1$eaf.outcome * (1-dat1$eaf.outcome) /
  (2*(dat1$beta.outcome)^2 * dat1$eaf.outcome * (1-dat1$eaf.outcome) + 
     (dat1$se.outcome)^2*2*700000*dat1$eaf.outcome * (1-dat1$eaf.outcome))
