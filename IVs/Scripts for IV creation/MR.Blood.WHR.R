...
#library(TwoSampleMR)
##library(MRPRESSO)
##library(dplyr) -- need if want Good Forest Plot at end, not  necessary for screen
##library(stringr) -- need  if want   Good Forest Plot at end, not  necessary	for screen
##library(ggplot2) -- need  if want   Good Forest Plot at end, not  necessary	for screen

#setwd("/project/voight_ML/thomc/thomc_results/MendelianRandomization/MR.Laptop.Backup.200502/MR.Smk.Blood.ASCVD/2020VuckovicBloodData/")

######################
#####Ran following commands initially to organize and clump exposure data. Cannot clump on cluster so this was annoying. Will load clumped data set, which will have common SNPs for all the VUCKOVIC  blood traits.

#See SmkInit or LfSmk -> Blood traits --> ASCVD in Benchling
#Initial test shows that SmkInit increases Hb
...
library(TwoSampleMR)
library(dplyr)
setwd("/project/voight_ML/thomc/thomc_results/MendelianRandomization/MR.Laptop.Backup.200502/MR.Smk.Blood.ASCVD/2020VuckovicBloodData/MR.WHR.Blood/")
...
EX <- read.table("/project/voight_ML/thomc/thomc_results/MendelianRandomization/MR.Laptop.Backup.200502/MR.Smk.Blood.ASCVD/2020VuckovicBloodData/WHR.HGB.Clumped", header=T)
EX$Phenotype <- "WHR"
EX_data <- format_data(EX, type="outcome", snps=EX$SNP, header=T,
...
#quick plt count one
#files <- list.files(path="/project/voight_ML/thomc/thomc_results/MendelianRandomization/MR.Laptop.Backup.200502/MR.Smk.Blood.ASCVD/test/", pattern="*ukbb_ukbil_meta.tsv.gz", full.names=FALSE, recursive=FALSE)
...
lapply(files, function(x) {
  ...
  t_data <- format_data(t, type="exposure", snps=t$SNP, header=T,
                       ...
                       other_allele_col="other_allele.y", pval_col="p-value.y")  # A2, P .... or other_allele and pval
    
  # clump and harmonize SmkInit_data to blood trait
  #SmkInit_data <- SmkInit_data %>% filter(SNP %in% t_data$SNP)
  #SmkInit_data_clump <- clump_data(SmkInit_data)
  dat1 <- harmonise_data(exposure_dat = t_data, outcome_dat = EX_data)
...
cat(sum(dat1$r2))
  ...
  write.table(dat1, paste0(trait, ".WHR.TwoSample.IV.txt"), quote=F,col.names=T,row.names=F,sep="\t")
  ...
  #print output
  write.table(res, paste0(trait, ".WHR.MR.txt"), quote=F,col.names=T,row.names=F,sep="\t")
  
  #heterogeneity test
  het <- mr_heterogeneity(dat1)
  write.table(het, paste0(trait, ".WHR.het.txt"), quote=F,col.names=T,row.names=F,sep="\t")
  #horiz pleiotropy test
  hp <- mr_pleiotropy_test(dat1)
  write.table(hp, paste0(trait, ".WHR.hp.txt"), quote=F,col.names=T,row.names=F,sep="\t")
  ...
  #Scatterplot
  pdf(paste0(trait, ".WHR.Scatterplot.pdf"))
  ...
  #pdf(paste0(trait, ".WHR.GoodForest.pdf"))
