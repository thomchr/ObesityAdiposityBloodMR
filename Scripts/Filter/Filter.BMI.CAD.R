...
out <- fread("/project/voight_datasets/GWAS/21_CADMeta/CAD_META", header=T)
out$Allele1 <- toupper(out$Allele1)
out$Allele2 <- toupper(out$Allele2)
out$Phenotype<- "CAD"

#e1_comm<- e1[e1$SNP%in% e2$SNP,] 
#e1_comm<- e1_comm[e1_comm$SNP%in% out$oldID,]
...
e2_comm<- e2[e2$SNP %in% out$oldID,]
out_comm<- out[out$oldID %in% e2_comm$SNP,]
