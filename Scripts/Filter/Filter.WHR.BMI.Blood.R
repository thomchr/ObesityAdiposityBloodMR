...
#e2$ChrPosRefAlt<- paste0(e2$CHR, ":", e2$POS, "_", e2$Tested_Allele, "_", e2$Other_Allele)
#e2$ChrPosAltRef<- paste0(e2$CHR, ":", e2$POS, "_", e2$Other_Allele, "_", e2$Tested_Allele)
...
e1_sig <- e1_comm %>% filter(P <= 5E-8)
e2_sig <- e2_comm %>% filter(P <= 5E-8)
write.table(e1_sig, "WHR.sig.txt", sep="\t", quote= FALSE, row.names= FALSE, col.names= TRUE)
write.table(e2_sig, "BMI.sig.txt", sep="\t", quote= FALSE, row.names= FALSE, col.names= TRUE)



quit()
#don't need the 2nd half yet

### 2nd part... Load for each blood trait, merge with e1_comm (TC.filtered.txt) because this has ChrPosRefAlt and ChrPosAltRef in it (which match rs_number in blood traits)
