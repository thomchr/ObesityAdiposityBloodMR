...
e1_comm <- fread("WHR.filtered.txt", header=T) #this is not used for anything in analysis, just to find rs_numbers from blood traits
e2_comm <- fread("BMI.filtered.txt", header=T)
e3_clump <- fread("WHR.Clumped", header=T) #for outcomes, WHR becomes e1
...
lapply(files, function(x) {
  ...
  t_clump <- t[t$SNP %in% e3_clump$SNP]
  out_clump <- format_data(t_clump, type="outcome", snps=t_clump$SNP, header=T,
...
write.table(combined.e1_e2, paste0("WHR.BMI.", trait, ".MVMR.IV.txt"), sep="\t", row.names=F, col.names=TRUE, quote=FALSE)
...
cat(paste0("WHR.BMI.", trait, ".MVMRIVW"))
...
write.table(dat1, paste0("WHR.", trait, ".TwoSample.IV.txt"), quote=F,col.names=T,row.names=F,sep="\t")


#run MR
res <- mr(dat1)
write.table(res,paste0("WHR.", trait, ".TwoSample.MR.txt"),quote=F,col.names=T,row.names=F,sep="\t")
het <- mr_heterogeneity(dat1)
write.table(het,paste0("WHR.", trait, ".TwoSample.heterogeneity.txt"),quote=F,col.names=T,row.names=F,sep="\t")
hp <- mr_pleiotropy_test(dat1)
write.table(hp,paste0("WHR.", trait, ".TwoSample.pleiotropy.txt"),quote=F,col.names=T,row.names=F,sep="\t")


##Plots
#Scatterplot
pdf(paste0("WHR.", trait, ".TwoSample.MR.Scatterplot.pdf"))
mr_scatter_plot(res, dat1)
dev.off()

#Forest plot
pdf(paste0("WHR.", trait, ".TwoSample.MR.Forestplot.pdf"))
...
#Leave One Out Plot
pdf(paste0("WHR.", trait, ".TwoSample.MR.LeaveOneOutplot.pdf"))
res_loo <- mr_leaveoneout(dat1)
mr_leaveoneout_plot(res_loo)
dev.off()

#Good forest plot with actual estimates
pdf(paste0("WHR.", trait, ".TwoSample.Good.forestplot.pdf"))
