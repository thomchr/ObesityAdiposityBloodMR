...
setwd("/project/voight_ML/thomc/thomc_results/MendelianRandomization/MR.Laptop.Backup.200502/MR.Smk.Blood.ASCVD/2020VuckovicBloodData/MVMR.BMI.RBC.WBC/")
...
e2 <- fread("/project/voight_datasets/GWAS/59_2020BloodTraits/BCX2_RBC_EA_GWAMA.out.gz", header=T)
e2$Phenotype <- "RBC"
...
out <- fread("/project/voight_datasets/GWAS/59_2020BloodTraits/BCX2_WBC_EA_GWAMA.out.gz", header=T)
out$Phenotype <- "WBC"
...
write.table(combined.e1_e2, paste0("BMI.RBC.WBC.MVMR.IV.txt"), sep="\t", row.names=F, col.names=TRUE, quote=FALSE)
...
cat(paste0("BMI.RBC.WBC.MVMRIVW"))
