...
out_comm <- fread("HDL.filtered.txt", header=T)

# find blood trait clumped files
files <- list.files(path="/project/voight_ML/thomc/thomc_results/MendelianRandomization/MR.Laptop.Backup.200502/MR.Smk.Blood.ASCVD/2020VuckovicBloodData/MVMR.Blood.WHR.BMI.HDL/", pattern="*Clumped", full.names=FALSE, recursive=FALSE)

lapply(files, function(x) {

  e3_clump <- fread(paste0("/project/voight_ML/thomc/thomc_results/MendelianRandomization/MR.Laptop.Backup.200502/MR.Smk.Blood.ASCVD/2020VuckovicBloodData/MVMR.Blood.WHR.BMI.HDL/", x), header=TRUE)
...
#write.table(out_clump, file = "HDL.Clumped", sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
...
write.table(combined.e1_e2_e3, paste0(trait, ".WHRBMI.HDL.MVMR.IV.txt"), quote=F,col.names=T,row.names=F,sep="\t")
...
write.table(dat1, paste0(trait, ".HDL.TwoSample.IV.txt"), quote=F,col.names=T,row.names=F,sep="\t")


#run MR
res <- mr(dat1)
write.table(res,paste0(trait,".HDL.TwoSample.MR.txt"),quote=F,col.names=T,row.names=F,sep="\t")
het <- mr_heterogeneity(dat1)
write.table(het,paste0(trait,".HDL.TwoSample.heterogeneity.txt"),quote=F,col.names=T,row.names=F,sep="\t")
hp <- mr_pleiotropy_test(dat1)
write.table(hp,paste0(trait,".HDL.TwoSample.pleiotropy.txt"),quote=F,col.names=T,row.names=F,sep="\t")


##Plots
#Scatterplot
pdf(paste0(trait,".HDL.TwoSample.MR.Scatterplot.pdf"))
mr_scatter_plot(res, dat1)
dev.off()

#Forest plot
pdf(paste0(trait,".HDL.TwoSample.MR.Forestplot.pdf"))
...
#Leave One Out Plot
pdf(paste0(trait,".HDL.TwoSample.MR.LeaveOneOutplot.pdf"))
res_loo <- mr_leaveoneout(dat1)
mr_leaveoneout_plot(res_loo)
dev.off()

#Good forest plot with actual estimates
pdf(paste0(trait,".HDL.TwoSample.Good.forestplot.pdf"))
