...
#e3_comm <- fread("WHR.filtered.txt", header=T) #for outcomes, WHR becomes e1
...
lapply(files, function(x) {
  ...
  t_clump <- t[t$SNP %in% e2_clump$SNP]
...
write.table(combined.e1_e2, paste0("BMI.WHR.", trait, ".MVMR.IV.txt"), sep="\t", row.names=F, col.names=TRUE, quote=FALSE)
...
cat(paste0("BMI.WHR.", trait, ".MVMRIVW"))
...
#run MR
res <- mr(dat1)
write.table(res,paste0("BMI.", trait, ".TwoSample.MR.txt"),quote=F,col.names=T,row.names=F,sep="\t")
het <- mr_heterogeneity(dat1)
write.table(het,paste0("BMI.", trait, ".TwoSample.heterogeneity.txt"),quote=F,col.names=T,row.names=F,sep="\t")
hp <- mr_pleiotropy_test(dat1)
write.table(hp,paste0("BMI.", trait, ".TwoSample.pleiotropy.txt"),quote=F,col.names=T,row.names=F,sep="\t")


##Plots
#Scatterplot
pdf(paste0("BMI.", trait, ".TwoSample.MR.Scatterplot.pdf"))
mr_scatter_plot(res, dat1)
dev.off()

#Forest plot
pdf(paste0("BMI.", trait, ".TwoSample.MR.Forestplot.pdf"))
...
#Leave One Out Plot
pdf(paste0("BMI.", trait, ".TwoSample.MR.LeaveOneOutplot.pdf"))
res_loo <- mr_leaveoneout(dat1)
mr_leaveoneout_plot(res_loo)
dev.off()

#Good forest plot with actual estimates
pdf(paste0("BMI.", trait, ".TwoSample.Good.forestplot.pdf"))
