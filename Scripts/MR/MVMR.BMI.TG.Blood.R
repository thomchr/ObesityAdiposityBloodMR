...
e3_comm <- fread("TG.filtered.txt", header=T) #for outcomes, TC becomes e1

#e1_clump <- e1_comm %>% filter(SNP %in% e2_clump$SNP)
...
#e2_clump <- e2_comm[e2_comm$SNP %in% e2_clump$SNP]
...
e3_clump <- e3_comm %>% filter(SNP_ID %in% e2_clump$SNP)
e3_clump <- format_data(e3_clump, type="exposure", snps=e3_clump$SNP_ID, header=T, phenotype_col = "Phenotype", 
  	      snp_col="SNP_ID", beta_col="Effect", se_col="SE", 
	      eaf_col="EAF", effect_allele_col="Allele1", other_allele_col="Allele2", pval_col="PValue")
write.table(e3_clump, file = "TG.Clumped", sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
...
lapply(files, function(x) {
  ...
  t_clump <- t[t$SNP_ID %in% e2_clump$SNP]
...
write.table(combined.e1_e2, paste0("BMI.TG.", trait, ".MVMR.IV.txt"), row.names=F, col.names=TRUE, quote=FALSE)
...
cat(paste0("BMI.TG.", trait, ".MVMRIVW"))
...
write.table(dat1, paste0("BMI.TG.", trait, ".TwoSample.IV.txt"), quote=F,col.names=T,row.names=F,sep="\t")


#run MR
res <- mr(dat1)
write.table(res,paste0("BMI.TG.", trait, ".TwoSample.MR.txt"),quote=F,col.names=T,row.names=F,sep="\t")
het <- mr_heterogeneity(dat1)
write.table(het,paste0("BMI.TG.", trait, ".TwoSample.heterogeneity.txt"),quote=F,col.names=T,row.names=F,sep="\t")
hp <- mr_pleiotropy_test(dat1)
write.table(hp,paste0("BMI.TG.", trait, ".TwoSample.pleiotropy.txt"),quote=F,col.names=T,row.names=F,sep="\t")


##Plots
#Scatterplot
pdf(paste0("BMI.TG.", trait, ".TwoSample.MR.Scatterplot.pdf"))
mr_scatter_plot(res, dat1)
dev.off()

#Forest plot
pdf(paste0("BMI.TG.", trait, ".TwoSample.MR.Forestplot.pdf"))
...
#Leave One Out Plot
pdf(paste0("BMI.TG.", trait, ".TwoSample.MR.LeaveOneOutplot.pdf"))
res_loo <- mr_leaveoneout(dat1)
mr_leaveoneout_plot(res_loo)
dev.off()

#Good forest plot with actual estimates
pdf(paste0("BMI.TG.", trait, ".TwoSample.Good.forestplot.pdf"))
