...
e3_clump <- fread("TC.Clumped", header=T) #for outcomes, TC becomes e1
...
write.table(combined.e1_e2, paste0("TC.BMI.", trait, ".MVMR.IV.txt"), row.names=F, col.names=TRUE, quote=FALSE)
...
write.table(dat1, paste0("TC.", trait, ".TwoSample.IV.txt"), quote=F,col.names=T,row.names=F,sep="\t")


#run MR
res <- mr(dat1)
write.table(res,paste0("TC.", trait, ".TwoSample.MR.txt"),quote=F,col.names=T,row.names=F,sep="\t")
het <- mr_heterogeneity(dat1)
write.table(het,paste0("TC.", trait, ".TwoSample.heterogeneity.txt"),quote=F,col.names=T,row.names=F,sep="\t")
hp <- mr_pleiotropy_test(dat1)
write.table(hp,paste0("TC.", trait, ".TwoSample.pleiotropy.txt"),quote=F,col.names=T,row.names=F,sep="\t")


##Plots
#Scatterplot
pdf(paste0("TC.", trait, ".TwoSample.MR.Scatterplot.pdf"))
mr_scatter_plot(res, dat1)
dev.off()

#Forest plot
pdf(paste0("TC.", trait, ".TwoSample.MR.Forestplot.pdf"))
...
#Leave One Out Plot
pdf(paste0("TC.", trait, ".TwoSample.MR.LeaveOneOutplot.pdf"))
res_loo <- mr_leaveoneout(dat1)
mr_leaveoneout_plot(res_loo)
dev.off()

#Good forest plot with actual estimates
pdf(paste0("TC.", trait, ".TwoSample.Good.forestplot.pdf"))
