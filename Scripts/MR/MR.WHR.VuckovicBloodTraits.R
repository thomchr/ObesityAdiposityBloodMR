...
EX <- read.table("WHR.HGB.Clumped", header=T)
EX$Phenotype <- "WHR"
...
lapply(files, function(x) {
  ...
  write.table(dat1, paste0("WHR.", trait, ".IV.txt"), quote=F,col.names=T,row.names=F,sep="\t")
  ...
  #print output
  write.table(res, paste0("WHR.", trait, ".MR.txt"), quote=F,col.names=T,row.names=F,sep="\t")
  
  #heterogeneity test
  het <- mr_heterogeneity(dat1)
  write.table(het, paste0("WHR.", trait, ".het.txt"), quote=F,col.names=T,row.names=F,sep="\t")
  #horiz pleiotropy test
  hp <- mr_pleiotropy_test(dat1)
  write.table(hp, paste0("WHR.", trait, ".hp.txt"), quote=F,col.names=T,row.names=F,sep="\t")
  ...
  #Scatterplot
  pdf(paste0("WHR.", trait, ".Scatterplot.pdf"))
