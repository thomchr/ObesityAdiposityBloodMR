...
setwd("/Users/thomc/Desktop/Smk.BMI.Blood.MR/MR.WHRadjBMI.Lfsmk/")
...
if( file.exists("LfSmk.filtered.txt") & file.exists("WHRadjBMI.filtered.txt")) { 
  ...
  out_comm <- fread("LfSmk.filtered.txt", header=T)
  ...
  out <- fread("/Volumes/4TB.RNAseq/GWASdata/2019.10.02.SummStats", header=T)
  out$Phenotype <- "LfSmk"
  ...
  write.table(out_comm, "LfSmk.filtered.txt", sep="\t", quote= FALSE, row.names = FALSE, col.names = TRUE)
}
...
if( file.exists("LfSmk.Clumped") & file.exists("WHRadjBMI.Clumped")) { 
  ...
  out_clump <- fread("LfSmk.Clumped", header=T)
...
write.table(out_clump, file = "LfSmk.Clumped", sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
