---
title: "MVMR.WHRadjBMI.SmkInit.VuckovicBlood"
author: "Chris Thom"
date: "2/1/21"
...
setwd("/Users/thomc/Desktop/Smk.BMI.Blood.MR/MVMR.WHRadjBMI.SmkInit.HGB/")
...
if( file.exists("SmkInit.filtered.txt") & file.exists("WHRadjBMI.filtered.txt") & file.exists("HGB.filtered.txt")) { 
  ...
  e2_comm <- fread("SmkInit.filtered.txt", header=T)
  ...
  e2 <- fread("/Volumes/4TB.RNAseq/GWASdata/SmokingInitiation.txt", header=T)
  e2$Phenotype <- "SmkInit"
  e2$ChrPosRefAlt <- paste0(e2$CHROM, ":", e2$POS, "_", e2$ALT, "_", e2$REF)
  e2$ChrPosAltRef <- paste0(e2$CHROM, ":", e2$POS, "_", e2$REF, "_", e2$ALT)
  

  out <- fread("/Volumes/4TB.RNAseq/GWASdata/BCX2_HGB_EA_GWAMA.out.gz", header=T)
  out$Phenotype <- "HGB"
  
  e2_comm <- e2[e2$RSID %in% e1$SNP,] 
  ...
  e1_comm <- e1[e1$SNP %in% e2_comm$RSID,]
  ...
  write.table(e2_comm, "SmkInit.filtered.txt", sep="\t", quote= FALSE, row.names = FALSE, col.names = TRUE)
...
e2_clump <- e2_comm[e2_comm$RSID %in% e1_clump$SNP]
e2_clump <- format_data(e2_clump, type="exposure", snps=e2_clump$RSID, header=T, phenotype_col = "Phenotype", 
