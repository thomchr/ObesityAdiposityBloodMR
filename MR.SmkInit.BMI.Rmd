...
setwd("/Users/thomc/Desktop/Smk.BMI.Blood.MR/MR.SmkInit.BMI/")
...
if( file.exists("SmkInit.filtered.txt") & file.exists("BMI.filtered.txt")) { 
  ...
  out <- fread("/Volumes/4TB.RNAseq/GWASdata/Meta-analysis_Locke_et_al+UKBiobank_2018_UPDATED.txt.gz", header=T)
  out$Phenotype <- "BMI"
  ...
  write.table(out_comm, "BMI.filtered.txt", sep="\t", quote= FALSE, row.names = FALSE, col.names = TRUE)
}
...
if( file.exists("SmkInit.Clumped") & file.exists("BMI.Clumped")) { 
  ...
  out_clump <- fread("BMI.Clumped", header=T)
...
write.table(out_clump, file = "BMI.Clumped", sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
