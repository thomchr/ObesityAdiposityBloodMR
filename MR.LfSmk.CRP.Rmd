...
setwd("/Users/thomc/Desktop/Smk.BMI.Blood.MR/MR.LfSmk.CRP/")
...
if( file.exists("LfSmk.filtered.txt") & file.exists("HGB.filtered.txt")) { 
  print("Working from pre-tabulated files")
  e1_comm <- fread("LfSmk.filtered.txt", header=T)
  out_comm <- fread("CRP.filtered.txt", header=T)
  ...
  e1_comm <- e1 %>% filter(SNP %in% out$variant_id)
  out_comm <- out %>% filter(variant_id %in% e1_comm$SNP)
...
if( file.exists("LfSmk.Clumped") & file.exists("CRP.Clumped")) { 
...
dat1$r2 <- 2*(dat1$beta.exposure)^2 * dat1$eaf.exposure * (1-dat1$eaf.exposure) /
  (2*(dat1$beta.exposure)^2 * dat1$eaf.exposure * (1-dat1$eaf.exposure) + 
     (dat1$se.exposure)^2*2*462690*dat1$eaf.exposure * (1-dat1$eaf.exposure))
