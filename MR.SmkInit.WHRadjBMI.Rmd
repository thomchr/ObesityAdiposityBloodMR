...
setwd("/Users/thomc/Desktop/Smk.BMI.Blood.MR/MR.SmkInit.WHRadjBMI/")
...
if( file.exists("SmkInit.filtered.txt") & file.exists("HGB.filtered.txt")) { 
  ...
  e1_comm <- e1 %>% filter(RSID %in% out$SNP)
  out_comm <- out %>% filter(SNP %in% e1_comm$RSID)
...
# sample size for CAD is 547261, SmkInit is 557337 per Dan Hui (paper says up to 1,232,091) --- Using Dan's estimate
