...
setwd("/Users/thomc/Desktop/Smk.BMI.Blood.MR/MR.LfSmk.WHRadjBMI/")
...
if( file.exists("LfSmk.filtered.txt") & file.exists("WHRadjBMI.filtered.txt")) { 
  print("Working from pre-tabulated files")
  e1_comm <- fread("LfSmk.filtered.txt", header=T)
  out_comm <- fread("WHRadjBMI.filtered.txt", header=T)
  
} else { 
  print("Loading summ stats and making filtered files")
  e1 <- fread("/Volumes/4TB.RNAseq/GWASdata/2019.10.02.SummStats", header=T)
  e1$Phenotype <- "LfSmk"

  out <- fread("/Volumes/4TB.RNAseq/GWASdata/whradjbmi.giant-ukbb.meta-analysis.combined.23May2018.txt", header=T)
  out$Phenotype <- "WHRadjBMI"
  out$SNP <- gsub("[A-Z]|:", "", out$SNP) #bc WHRadjBMI has rsID:Allele:Allele
  
  e1_comm <- e1 %>% filter(SNP %in% out$SNP)
  out_comm <- out %>% filter(SNP %in% e1_comm$SNP)
  
  write.table(e1_comm, "LfSmk.filtered.txt", sep="\t", quote= FALSE, row.names = FALSE, col.names = TRUE)
  write.table(out_comm, "WHRadjBMI.filtered.txt", sep="\t", quote= FALSE, row.names = FALSE, col.names = TRUE)
}
...
if( file.exists("LfSmk.Clumped") & file.exists("WHRadjBMI.Clumped")) { 
  print("Working from pre-tabulated clump files")
  e1_clump <- fread("LfSmk.Clumped", header=T)
  out_clump <- fread("WHRadjBMI.Clumped", header=T)
...
write.table(e1_data, file = "LfSmk.Clumped", sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
...
write.table(out_clump, file = "WHRadjBMI.Clumped", sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
...
# sample size for CAD is 547261, SmkInit is 557337 per Dan Hui (paper says up to 1,232,091) --- Using Dan's estimate, LfSmk is 462690
dat1$r2 <- 2*(dat1$beta.outcome)^2 * dat1$eaf.outcome * (1-dat1$eaf.outcome) /
  (2*(dat1$beta.outcome)^2 * dat1$eaf.outcome * (1-dat1$eaf.outcome) + 
     (dat1$se.outcome)^2*2*462690*dat1$eaf.outcome * (1-dat1$eaf.outcome))
